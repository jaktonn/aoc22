...
fun letterPriority(letter: Char): Int {
...
fun main() {
    val splitList = input.map { line -> listOf(
        line.substring(0 until line.length/2),
        line.substring(line.length/2 )
    ) }
    println(splitList)
    val commonLetters = splitList.map { parts -> parts[0].toSet().intersect(parts[1].toSet()).first() }
    println(commonLetters)
    val priorities = commonLetters.map { letter -> letterPriority(letter) }
    println(priorities)
    ...
    while (inputIterator.hasNext()) {
       val groupLetter = inputIterator.next().toSet().intersect(inputIterator.next().toSet()).intersect(inputIterator.next().toSet())
       groups.add(groupLetter.first())
    }
    println(groups)
    val groupPriorities = groups.map { letter -> letterPriority(letter) }
    println(groupPriorities)
    println("### Sum of group priorities ${groupPriorities.sum()}")
}